...
#    bob$cdf <-  P(unlist(bob[,method]))
    ...
    P <- ecdf(unlist(bob[c(whichbob_neut),method]))
      # creates a probability function based on neutral loci
    
    cdf_neut <- mean(P(unlist(bob[whichbob_neut,method])))
    cdf_LEQTL <- P(unlist(bob[whichbob_LEQTL,method]))
    cdf_Inversion <- mean(P(unlist(bob[whichbob_Inversion,method])), na.rm=TRUE)
    cdf_fullSS <- mean(P(unlist(bob[whichbob_fullSS,method])), na.rm=TRUE)
    cdf_partSS <- mean(P(unlist(bob[whichbob_partSS,method])), na.rm=TRUE)
    cdf_lowR <-  mean(P(unlist(bob[whichbob_lowR,method])), na.rm=TRUE)
...
pdf("figures/AUC-pr.pdf", width=6, height=10)
  ...
  outstat <- boxplot(perf_df2a$AUCpr_fullSS~perf_df2a$name, las=2, names=NA,
          main = "A) Full sweep", col=as.character(plot_df$color[-2]),
          ylim=c(0,1), density=20)
  ...
  outstat1 <- boxplot(perf_df2a$AUCpr_partSS~perf_df2a$name, las=2, names=NA,
          main = "B) Partial sweep", col=as.character(plot_df$color[-2]),
          ylim=c(0,1), density=20)
  ...
  outstat2 <- boxplot(perf_df2a$AUCpr_QTN~perf_df2a$name, las=2, 
          main = "C) Causal QTNs",  ylim=c(0,1), col=as.character(plot_df$color[-2]))
...
```{r}
## How are sweep loci affected by allele freq at sweep or inversion? ####
topl <- perf_df3 %>% filter(name %in% c("H12",  "-log(H2/H1)", "iHS"))
head(topl)
unique(topl$name)

topl2 <- gather(topl, "cdf_type", "quantile", cdf_fullSS, cdf_partSS, cdf_Inversion, cdf_lowR)

head(topl2)
str(topl2)
topl2$cdf_type <- as.factor(topl2$cdf_type)
levels(topl2$cdf_type)
topl3 <- topl2 %>% mutate(cdf_type2 = recode_factor(cdf_type, 
                                        "cdf_partSS" = "Partial sweep",
                                        "cdf_fullSS" = "Full sweep",
                                        "cdf_Inversion" = "Inversion",
                                        "cdf_lowR" = "Low recomb.", 
                                        .ordered=TRUE
                                        ))
levels(topl3$cdf_type2)

### Make a plot of quantile as a function of region for sweeps ####

pdf("figures/Quantile_region_sweep_inversion.pdf", width=6, height=4)
ggplot(data=topl3, aes(x=name, y=quantile, fill=cdf_type2)) + geom_boxplot()  +  
  ylim(0,1) +
  theme_classic(base_size = 15) +
  scale_fill_manual(values=c("#064273", "#1da2d8", "#ffdc6f", "#fffa82"), name="Region") + 
  labs(x="", y="Quantile relative to neutral loci") +
...
```{r check inversion haplotype freq vs H12}
pdf("figures/inversionMHFvsHstatsCDF.pdf", width=6, height=5)
par(mfrow=c(1,1), mar=c(4,5,0.5,0.5), oma=c(0,0,0,0))

xz <- topl$af_inv[topl$name=="H12"]
xz[xz>0.5] = 1-xz[xz>0.5]
plot(xz, topl$cdf_Inversion[topl$name=="H12"], xlim=c(0, 0.5), ylab="Quantile in inversion \nrelative to neutral loci",
     xlab="Minor haplotype frequency", bty="l", pch=19, col=rgb(0.5,0,1,0.3),
     ylim=c(0,1))

  xy<- data.frame(x=xz,
                  y=topl$cdf_Inversion[topl$name=="H12"])
  xy <- xy[order(xy$x),]
  out <- lm(xy$y ~ poly(xy$x,1))
  xy$y2 <- predict(out)
  points(xy$x, xy$y2, type="l", col=rgb(0.5,0,1), lwd=3)

  
  xz <- topl$af_inv[topl$name=="-log(H2/H1)"]
  xz[xz>0.5] = 1-xz[xz>0.5]  
  points(xz, topl$cdf_Inversion[topl$name=="-log(H2/H1)"], xlim=c(0, 0.5), ylab="Quantile in inversion \nrelative to neutral loci",
     xlab="Minor haplotype frequency in inversion", bty="l", pch=3, col=rgb(0.5,0.5,0, 0.6),
     ylim=c(0,1))
  xy<- data.frame(x=xz,
                  y=topl$cdf_Inversion[topl$name=="-log(H2/H1)"])
  xy <- xy[order(xy$x),]
  out <- lm(xy$y ~ poly(xy$x,1))
  xy$y2 <- predict(out)
  points(xy$x, xy$y2, type="l", col=rgb(0.5,0.5,0), lwd=3)
  
  text(0.15,0.9,"H12", col=rgb(0.5,0,1), cex=1.5)
  text(0.15,0.6,"-log(H2/H1)", col=rgb(0.5,0.5,0), cex=1.5)
 dev.off() 
```
...
```{r}
### Make a plot of quantile as a function of a.f. for sweeps ####
pdf("figures/Quantile_sweep_vs_af.pdf", width=10, height=4)

par(mfrow=c(1,2), mar=c(4,5,0.5,0.5), oma=c(0,0,0,0))

plot(topl$af_fullSS[topl$name=="H12"], topl$cdf_fullSS[topl$name=="H12"], xlim=c(0, 1.0), ylab="Quantile in sweep regions\nrelative to neutral loci",
     xlab="Frequency of sweep", bty="l", pch=19, col=rgb(0.5,0,1,0.3),
     ylim=c(0,1))

abline(h=0.5, col="grey")

  points(topl$af_partSS[topl$name=="H12"], topl$cdf_partSS[topl$name=="H12"], pch=21, col=rgb(0.5,0,1,0.3))
  xy<- data.frame(x=c(topl$af_fullSS[topl$name=="H12"], topl$af_partSS[topl$name=="H12"]),
                  y=c(topl$cdf_fullSS[topl$name=="H12"], topl$cdf_partSS[topl$name=="H12"]))
  xy <- xy[order(xy$x),]
  out <- lm(xy$y ~ poly(xy$x,3))
  xy$y2 <- predict(out)
  points(xy$x, xy$y2, type="l", col=rgb(0.5,0,1), lwd=3)
  
    points(topl$af_partSS[topl$name=="iHS"], topl$cdf_partSS[topl$name=="iHS"], pch=2, col=rgb(0,0.5,1,0.3))
    points(topl$af_fullSS[topl$name=="iHS"], topl$cdf_fullSS[topl$name=="iHS"], pch=17, col=rgb(0,0.5,1,0.3))
      xy<- data.frame(x=c(topl$af_fullSS[topl$name=="iHS"], topl$af_partSS[topl$name=="iHS"]),
                  y=c(topl$cdf_fullSS[topl$name=="iHS"], topl$cdf_partSS[topl$name=="iHS"]))
   xy <- xy[order(xy$x),]
    out <- lm(xy$y ~ poly(xy$x,3))
    xy$y2 <- predict(out)
    points(xy$x, xy$y2, type="l", col=rgb(0,0.5,1), lwd=3)
    
  text(0.3, 0.85, "H12", cex=1.5, col=rgb(0.5,0,1))
  text(0.45, 0.65, "iHS", cex=1.5, col=rgb(0,0.5,1))
  legend("bottomleft", c("H12 partial SS", "H12 full SS", "iHS partial SS", "iHS full SS"),
         pch=c(21, 19, 2, 17), col=c(rgb(0.5,0,1),rgb(0.5,0,1), rgb(0,0.5,1),rgb(0,0.5,1)),
         bty="n", cex=1, x.inters=1)
  text(0, 1, "A", cex=1.5)
## end first plot
  
## check out gen since fixation
  par(mar=c(4,2,0.5,0.5))
    topl$fixed_since <- as.numeric(topl$fixed_since)
  summary(topl$fixed_since)
  cond <- topl$name=="H12" & topl$fixed_since>1 & !is.na(topl$fixed_since)
  plot(topl$fixed_since[cond], topl$cdf_fullSS[cond], ylim=c(0, 1.0), ylab="Quantile in sweep regions\nrelative to neutral loci",
     xlab="Generations since fixation", bty="l", pch=19, col=rgb(0.5,0,1,0.3), xlim=c(0,200))
      xy<- data.frame(x=topl$fixed_since[which(cond)], y= topl$cdf_fullSS[which(cond)])
   xy <- xy[order(xy$x),]
    out <- lm(xy$y ~ poly(xy$x,1))
    xy$y2 <- predict(out)
    points(xy$x, xy$y2, type="l", col=rgb(0.5,0,1), lwd=3)
  ...
  cond <- topl$name=="iHS" & topl$fixed_since>1 & !is.na(topl$fixed_since)
  points(topl$fixed_since[cond], topl$cdf_fullSS[cond], ylim=c(0, 1.0), ylab="Quantile in sweep regions\nrelative to neutral loci",
     xlab="Generations since fixation", bty="l", pch=19, col=rgb(0,0.5,1,0.3), xlim=c(0,200))
      xy<- data.frame(x=topl$fixed_since[which(cond)], y= topl$cdf_fullSS[which(cond)])
   xy <- xy[order(xy$x),]
    out <- lm(xy$y ~ poly(xy$x,1))
    xy$y2 <- predict(out)
...
## Quantiles for different regions for QTNs
```{r}
levels(perf_df3$name)

## How are QTL loci affected by  inversion? ####
topl <- perf_df3 %>% filter(name %in% c("PCAdapt Naive",  "PCAdapt BP", "OutFLANK Naive",
                                        "OutFLANK BP", "XTX Naive", "XTX BP"))
head(topl)
unique(topl$name)

topl2 <- gather(topl, "cdf_type", "quantile", cdf_LEQTL, cdf_Inversion, cdf_lowR)
...
a<- ggplot(data=topl3, aes(x=name, y=quantile, fill=cdf_type2, color=cdf_type2)) + geom_boxplot()  +  
  ylim(0,1.19) +
  theme_classic(base_size = 15) +
  scale_fill_manual(values=c("#4c004d", "#ffdc6f", "#fffa82"), name="Region") + 
  scale_color_manual(values=c("#b2458d", "black", "black"), name="Region") +
  labs(x="", y="Quantile relative to neutral loci") +
  geom_hline(yintercept=0.5, linetype="solid", color = "grey") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  annotate(geom="text", x=0.6, y=1.15, label="A", size=10)
#a

levels(perf_df3$name)

## How are QTL loci affected by inversion? ####
topl <- perf_df3 %>% filter(name %in% c("LFMM ridge (pheno)",    "LFMM lasso (pheno)",   
"LEA (envi)",            "Spearman's rho (envi)", "BayPass (envi) Naive",  "BayPass (envi) BP",    
"RDA (envi) loading"   ))
head(topl)
unique(topl$name)
...
b<- ggplot(data=topl3, aes(x=name, y=quantile, fill=cdf_type2, color=cdf_type2)) + geom_boxplot()  +  
  ylim(0,1.19) +
  theme_classic(base_size = 15) +
  scale_fill_manual(values=c("#4c004d", "#ffdc6f", "#fffa82"), name="Region") + 
  scale_color_manual(values=c("#b2458d", "black", "black"), name="Region") +
  ...
  annotate(geom="text", x=0.6, y=1.15, label="B", size=10)
#


### Make a plot of quantile as a function of region for sweeps ####
library(gridExtra)
pdf("figures/Quantile_region_QTL_inversion.pdf", width=8, height=12)
...
MakeAUCPRcurve <- function(thisim, let){  
  ...
  todo <- which(names(prdf) %in% c("pcadapt_3.0.4_PRUNED_log10p",
  ...
  for (i in 1:length(todo)){
    ...
    if(i==1){
      plot(wpr$curve[,1], wpr$curve[,2], col="purple", ylim=c(0,1.2), type="l", xlim=c(0,1.1),
    ...
    aucz=round(aucz,2)
    legend(0.5,1.2, c(paste("PCAdapt AUC = ",aucz[1]), 
...
pdf("figures/AUC-pr-curves.pdf", width=4, height=8)
  par(mar=c(4,6,6,1), mfrow=c(3,1), oma=c(0,0,0,0))
MakeAUCPRcurve(10944, "A") # pcadapt 
MakeAUCPRcurve(10956, "B") # outflank and baypass
MakeAUCPRcurve(10961, "C") # outflank and baypass 
  #10952 all low
   #10956 baypass
dev.off()
```
