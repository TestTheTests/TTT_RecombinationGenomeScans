...
RDA_files = paste0(inver_simID, "_Invers_RDA_loadings_envi.txt")
...
for (i in 1:length(inver_files)){
  ...
  ## RDA results
  x1_rda <- read.table(paste0("results_final/", RDA_files[i]), header=TRUE)
    if (sum(x1$keep_loci)!=nrow(x1_rda)){
    print("Error: wrong number of loci in RDA file"
    )
  }
  x1_rda$unique <- x1$unique[which(x1$keep_loci)]
  ...
  x_final3 <- left_join(x_final2, x1_rda, by="unique")
  x_final <- x_final3
...
## Deleterious allels
```{r deleterious}
summary(tapply(x_all$muttype=="MT=3",x_all$simID, sum))
```
...
pdf("figures/WhichLocationsPCs.pdf", width=9, height=6)  
  ...
  plot(d, main="A) Locations of outlier PC1 loadings (all SNPs)", bty="n", las=1, ylim=c(0, 13e-05), xlim=c(0,5e05),adj=0,
       ylab="", xlab="")
  polygon(d, col="black")
  ...
  d <- density(x_all$pos[which(abs(x_all$pca_ALL_PC2_loadings)>3)], bw = 1000)
    plot(d, main="B) Locations of outlier PC2 loadings (all SNPs)", bty="n", las=1, ylim=c(0, 6e-05), xlim=c(0,5e05), adj=0,
       ylab="", xlab="")
  polygon(d, col="black")
  plot_layers(y_head=10, y_arrows=c(10, 8), thisSim = thisim)
...
### PC from RDA
```{r}
quantile(abs(x_all$RDAvegan_v2.5.2_ALL_loading_PCA1_envi), 0.95, na.rm = TRUE)
quantile(abs(x_all$RDAvegan_v2.5.2_ALL_loading_PCA2_envi), 0.95, na.rm = TRUE)
quantile(abs(x_all$RDAvegan_v2.5.2_ALL_loading_PCA3_envi), 0.95, na.rm = TRUE)

 ### which locations load onto PC all data ####
pdf("figures/WhichLocationsPCs_RDA.pdf", width=9, height=6)  
  par(mar=c(4, 4,1,1), oma=c(0,2,0,0), mfrow=c(3,1))
  d <- density(x_all$pos[which(abs(x_all$RDAvegan_v2.5.2_ALL_loading_PCA1_envi)>0.08)], bw = 1000)
  ...
  d <- density(x_all$pos[which(abs(x_all$RDAvegan_v2.5.2_ALL_loading_PCA2_envi)>0.09)], bw = 1000)
    plot(d, main="B) Locations of outlier PC2 loadings in RDA (all SNPs)", bty="n", las=1, ylim=c(0, 3e-05), xlim=c(0,5e05), adj=0,
       ylab="", xlab="")
  polygon(d, col="black")
  plot_layers(y_head=10, y_arrows=c(10, 8), thisSim = thisim)
  
  d <- density(x_all$pos[which(abs(x_all$RDAvegan_v2.5.2_ALL_loading_PCA3_envi)>0.09)], bw = 1000)
...
```{r}
thisim = 10944
# 10944 and 10946 This is an interesting case when the inversion drifted to some diverse frequencies in a couple of the subpopulations, and resulted in a slightly elevated signal in the inversion. Note that this did not happen in many of the simulations.
# 10945, an interesting comparison.
...
makeManhattans <- function(thisim){
  ...
  par(mfrow=c(2,1), mar=c(0.5, 4, 2,0.1), oma=c(3,0,0,0))
  ...
  par(mfrow=c(6,1), mar=c(0.5, 4, 2,0.1), oma=c(3,0,0,0))
    ...
    plot(forpdf$pos, forpdf$OutFLANK_0.2_ALL_log10p,
       ylim= c(0, 24), pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i",
       cex=0.5, ylab= "-log10(P) OutFLANK Naive", xaxt="n", type="h", bty="l", main="C) OutFLANK (Fst) Naive", adj=0)
    plot_layers(y_head=20, y_arrows=c(13.5+4,11+4), thisSim=thisim)
    ...
    plot_layers(y_head=160, y_arrows=c(13.5+4,11+4), thisSim=thisim)
    
    plot(forpdf$pos, forpdf$baypass_2.1_ALL_XTX,
       ylim= c(25, 175), pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i",
  ...
  plot(forpdf$pos, abs(forpdf$Spearmans_ALL_rho),
       ylim= c(0, 0.9), pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i", type="h", bty="l", main="C) Spearman's rho (environment)", adj=0, ylab="     abs(Spearman's rho)", xaxt="n")
  plot_layers(y_head=0.7, y_arrows=c(0.6, 0.5), thisSim=thisim)
  
  plot(forpdf$pos, forpdf$LEA_1.2.0_ALL_K3_log10p,
       ylim= c(0, 90), pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i", type="h", bty="l", main="D) LEA (environment)", adj=0, ylab="        -log10 P (LEA)", xaxt="n")
  plot_layers(y_head=-10, y_arrows=c(90, 75), thisSim=thisim)
  ...
  plot_layers(y_head=70, y_arrows=c(60, 45), thisSim=thisim)
  
  plot(forpdf$pos, forpdf$baypass_2.1_PRUNED_BF_env,
       ylim= c(-20, 90), pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i", type="h", bty="l", main="F) BayPass Bayes Factor - Best Practice (environment)", adj=0, ylab="               Bayes Factor B.P.", xaxt="n")
  plot_layers(y_head=-40, y_arrows=c(60, 45), thisSim=thisim)
  
  plot(forpdf$pos, abs(forpdf$RDAvegan_v2.5.2_ALL_loading_RDA1_envi),
       ylim= c(0, 0.35), pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i", type="h", bty="l", main="G) RDA (environment)", adj=0, ylab="        loading (RDA)")
  plot_layers(y_head=-10, y_arrows=c(0.32, 0.25), thisSim=thisim)
...
makeManhattans(10944)
makeManhattans(10945)
makeManhattans(10946)
```
...
x_all <- x_all %>% mutate(RDAvegan_v2.5.2_ALL_loading_RDA1_envi_abs = abs(RDAvegan_v2.5.2_ALL_loading_RDA1_envi), Spearmans_ALL_rho_abs = abs(Spearmans_ALL_rho))
...
togetcols <- c(
  ...
  ## Assoc methods
      ...
      "RDAvegan_v2.5.2_ALL_loading_RDA1_envi_abs")
```
...
for (j in 1:length(togetcols)){
  ...
  AUCroc_SS <- as.numeric(unlist(auc.perf_SS@y.values))
...
### AUC- PR calculations for SS
  getAUCpr_SS <- function(x){
    selSS <- predict_list[[x]][label_list_SS[[x]]]
    selSS <- selSS[!is.na(selSS)]
    neutSS <- predict_list[[x]][!label_list_SS[[x]]]
    neutSS <- neutSS[!is.na(neutSS)]
    AUCpr_SS_tot <- pr.curve(scores.class0 = selSS, 
                          scores.class1 = neutSS, 
                          rand.compute = TRUE
                          )
    return(data.frame(AUCpr_SS = AUCpr_SS_tot$auc.integral,
    AUCpr_SS_rand = AUCpr_SS_tot$rand$auc.integral))
  }
    AUCpr_SS_final <- matrix(unlist(lapply(1:length(predict_list), getAUCpr_SS)), nrow=length(predict_list), ncol=2, byrow=TRUE)
  colnames(AUCpr_SS_final) <- c("AUCpr_SS", "AUCpr_SS_rand")

  
    
    perf_df <- rbind(perf_df, cbind(data.frame(method=togetcols[j], AUCroc_QTN =AUCroc_QTN , AUCroc_SS=AUCroc_SS, simID = inver_simID), AUCpr_QTN_final, AUCpr_SS_final))
} # end loop through
...
perf_df2 <- perf_df %>%
  ...
  ## Assoc methods
      ...
      RDAvegan_v2.5.2_ALL_loading_RDA1_envi_abs = "RDA (envi) loading"
      ) # end recode
   )
```
...
plot_df <- plot_df %>% mutate(color = recode(method,
  ...
  ## Assoc methods
      ...
      RDAvegan_v2.5.2_ALL_loading_RDA1_envi_abs = "#808000"
      )# end recode
) 
...
tail(perf_df2)
  ...
  dval = c(0,0,10,0,10,0,10,0,0,0,0,0,10,0, 0) # If adding new results, may need to edit this line. Naive approach should be hatched (e.g. 10)
  aval=45
```
...
pdf("figures/AUC-ROC.pdf", width=6, height=10)
  ...
  outstat <- boxplot(perf_df2$AUCroc_SS~perf_df2$name, las=2, names=NA,
...
boxplot(perf_df2$AUCpr_SS_rand~perf_df2$name)
boxplot(perf_df2$AUCpr_QTN_rand~perf_df2$name)

pdf("figures/AUC-pr.pdf", width=6, height=10)
  par(mar=c(1, 1, 1,1), mfrow=c(2, 1), oma=c(10, 4,0,0))
  
  outstat <- boxplot(perf_df2$AUCpr_SS~perf_df2$name, las=2, names=NA,
          main = "A) Global sweep", col=as.character(plot_df$color),
          ylim=c(0,1), density=20)
  
  rect((1:ngroups)-.4, outstat$stats[2,], (1:ngroups)+.4, outstat$stats[4,],
       density=dval, angle=aval)
  polygon(c(0,100,100,0),c(summary(perf_df2$AUCpr_SS_rand)[1],
                           summary(perf_df2$AUCpr_SS_rand)[1],
                           summary(perf_df2$AUCpr_SS_rand)[6],
                           summary(perf_df2$AUCpr_SS_rand)[6]),
          border=NA, col="grey")
  
  
  outstat2 <- boxplot(perf_df2$AUCpr_QTN~perf_df2$name, las=2, 
          main = "B) Causal QTNs",  ylim=c(0,1), col=as.character(plot_df$color))
  polygon(c(0,100,100,0),c(summary(perf_df2$AUCpr_QTN_rand)[1],
                           summary(perf_df2$AUCpr_QTN_rand)[1],
                           summary(perf_df2$AUCpr_QTN_rand)[6],
                           summary(perf_df2$AUCpr_QTN_rand)[6]),
          border=NA, col="grey")
  rect((1:ngroups)-.4, outstat2$stats[2,], (1:ngroups)+.4, outstat2$stats[4,],
       density=dval, angle=aval)
...
loop through columns
  ...
  #roc.curve(scores.class0 = x, scores.class1 = y, curve = TRUE)
  #get curve from wpr
  add points(wpr)
```
...
pdf("figures/Quantiles_other.pdf", width=6, height=10)
  ...
  outstat <- boxplot(perf_df3$cdf_SS*100~perf_df3$name, las=2, names=NA,
  ...
  outstat2 <- boxplot(perf_df3$cdf_BS*100~perf_df3$name, las=2, 
          main = "C) Background selection (LG 5 and 6)",  ylim=c(0,100), adj=0, col=as.character(plot_df$color))
  abline(h=50)
  rect((1:ngroups)-.4, outstat2$stats[2,], (1:ngroups)+.4, outstat2$stats[4,],
       density=dval, angle=aval)
