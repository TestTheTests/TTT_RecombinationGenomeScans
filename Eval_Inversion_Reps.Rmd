...
sum(is.na(x_all$simID))
  ...
  print.data.frame(x_all[is.na(x_all$a_freq_final),c("pos","a_freq_old", "descrip", "originGen", "seed")])
...
bob[which(bob>0.5)] <- bob[which(bob>0.5)]-0.5
pdf("figures/InversionMAF_hist.pdf", width=5, height=4)
  par(mar=c(4,4, 0.1,0.1), mfrow=c(1,1), oma=c(0,0,0,0))
  hist(bob, xlim=c(0,0.5), main=NULL, breaks=seq(0,0.5, by=0.01), xlab="Minor allele frequency of inversion", ylab="Number of simulations")
dev.off()
```
...
x_all %>% filter(seed==thisim, descrip=="Inversion")  %>% select(a_freq_final)

forpdf <- x_all[x_all$simID==thisim,]

(inv_loc <- forpdf$pos[which(forpdf$descrip=="Inversion")+1]) # this gives position within inversion
