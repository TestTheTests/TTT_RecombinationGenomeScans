...
# For each simulation, let's see how much genetic variance remains unexplained by causal loci that are not in the final dataset, but contribute more than 1% to AGV
  ...
  # on average 5% of genetic variance remains unexplained by causal loci not in the final dataset
...
pdf("figures/InversionMAF_hist.pdf", width=5, height=4)
  ...
  hist(bob, xlim=c(0,0.5), main=NULL, breaks=seq(0,0.5, by=0.05), xlab="Minor allele frequency at inversion", ylab="Number of simulations")
dev.off()
```
...
thisim = 10944
# 
names(x_all) <- gsub("sk-a","ska", names(x_all))
names(x_all) <- gsub("H2/H1","H2H1", names(x_all))
names(x_all)
...
makeManhattans <- function(thisim){
  ...
  plot(forpdf$pos[forpdf$keep_loci], forpdf$skallel_v1.1.10_H12[forpdf$keep_loci],
       ...
       ylim=c(0,1.5))#, 
       #xlim=c(170000 , 180000))
    plot_layers(y_head=12000, y_arrows=c(1.4, 1), thisSim = thisim)
    
  plot(forpdf$pos[forpdf$keep_loci], forpdf$skallel_v1.1.10_H2H1[forpdf$keep_loci],
        pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i",
       cex=0.7, ylab= "H2/H1 (H-scan)", xaxt="n",  type="h", bty="l",
       main="A) H2/H1", adj=0,
       ylim=c(0,1.5))#, 
       #xlim=c(170000 , 180000))
    plot_layers(y_head=12000, y_arrows=c(1.4, 1), thisSim = thisim)
    ...
    plot(forpdf$pos[forpdf$keep_loci], forpdf$skallel_v1.1.10_tajD[forpdf$keep_loci],
       ...
       ylim=c(-1,4))#, 
       #xlim=c(170000 , 180000))
    plot_layers(y_head=12000, y_arrows=c(4, 3.5), thisSim = thisim)
    
    plot(forpdf$pos[forpdf$keep_loci], forpdf$skallel_v1.1.10_ihs[forpdf$keep_loci],
        pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i",
       cex=0.7, ylab= "  -log10(P) iHS",  type="h", bty="l",
       main="B) iHS", adj=0,
       ylim=c(-3,4))
    plot_layers(y_head=90, y_arrows=c(4, 3), thisSim=thisim)
    
    #plot(forpdf$pos, forpdf$skallel_v1.1.10_pi,
    #    pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i",
    #   cex=0.7, ylab= "pi",  type="h", bty="l",
    #   main="B) pi", adj=0, ylim=c(0, 0.01))
    ...
    plot(forpdf$pos, forpdf$skallel_v1.1.10_nsl,
        pch=19, col=rgb(0,0,0,0.5), xaxs="i", yaxs="i",
       cex=0.7, ylab= "Theta W",  type="h", bty="l",
       main="B) Theta W", adj=0, ylim=c(-3, 4))
    plot_layers(y_head=90, y_arrows=c(4, 3), thisSim=thisim)
