...
pdf("figures/AUC-pr.pdf", width=6, height=10)
  par(mar=c(1, 1, 1,1), mfrow=c(3, 1), oma=c(10, 4,0,0))
  
  outstat <- boxplot(perf_df2$AUCpr_fullSS~perf_df2$name, las=2, names=NA,
          main = "A) Full sweep", col=as.character(plot_df$color),
          ylim=c(0,1), density=20)
  
  rect((1:ngroups)-.4, outstat$stats[2,], (1:ngroups)+.4, outstat$stats[4,],
       density=dval, angle=aval)
  polygon(c(0,100,100,0),c(summary(perf_df2$AUCpr_fullSS_rand)[1],
                           summary(perf_df2$AUCpr_fullSS_rand)[1],
                           summary(perf_df2$AUCpr_fullSS_rand)[6],
                           summary(perf_df2$AUCpr_fullSS_rand)[6]),
          border=NA, col="grey")
  
  boxplot(perf_df2$AUCpr_partSS~perf_df2$name, las=2, names=NA,
