...
vcf <- read.vcfR(paste(folder, seed, "_RecomLowReg_VCFallsim1.vcf.gz", sep=""))

ind0 <- read.table(paste(folder, seed, "_RecomLowReg_outputIndAll.txt", sep=""), header=TRUE)
muts <- read.table(paste(folder, seed, "_RecomLowReg_outputMuts.txt", sep=""), header=TRUE)
phen_env <-  read.table(paste(folder, seed, "_RecomLowReg_outputPhenEnv.txt", sep=""), header=TRUE)
sim <- system(paste("grep recomb ", folder, seed, "_RecomLowReg_outputSim.txt", sep=""), TRUE)
H <- read.table(paste(folder, seed, "_RecomLowReg_H_trial.txt", sep=""), header=TRUE)
```
...
for (i in 1:length(nlociperchrom)){
  ...
  #head(hapt_out[,1:10])
  write.table(cbind(rownames(hapt_out), hapt_out+1), paste0(temp, "chrom",i,".thap"), row.names=F, col.names=F, quote = FALSE)
}
...
write.table(map, "temp/map.inp", row.names=F, col.names=F)
```
...
for(i in 1:length(nlociperchrom)){
 cnt=cnt+1
 tmp.hapfile=paste0(temp, "chrom",i,".thap")
 
 tmp.hap=data2haplohh(hap_file=tmp.hapfile, map_file="temp/map.inp", chr.name=i,haplotype.in.columns=FALSE)
