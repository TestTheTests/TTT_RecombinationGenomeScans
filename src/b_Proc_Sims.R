...
seed <- args[1] #seed <- 10911
type <- args[2] #type<-"Invers"


### read files ####
folder <- "results/"
#setwd("/Users/katie/Desktop/Repos/TestTheTests/TTT_RecombinationGenomeScans/results")
# this script will be called from the "results" folder
...
### Mutation table ####
#head(muts)
if(sum(duplicated(muts$position))>0){
  print("Error: multiple selected mutations at same site")
  break
}
muts <- muts[-which(muts$freq==1.0 & muts$descrip=="QTL"),]
muts
### Fix "CHROM" in VCF ####
# Output "CHROM" in VCF from SliM does not show where the 
# recombination breakpoints are.
# Also output is sometimes unordered
...
# The inversion tracking mutation was reintroduced if lost due to drift
# (which it almost always was)
# If it was lost and was reintroduced, genotypes are 0 and 2
# If it was lost and reintroduced, but then rose to fixation,it was reintroduced again and genotypes are 0 and 3
...
### Calculate allele frequency before removing individuals ####
a_freq <- rowSums(G)/(2*ncol(G))

# If Inversion is rare, stop here and don't use simulation
if (a_freq[invloc]<0.01 | a_freq[invloc]>0.99){
  print("Inversion too rare to proceed"); break
}
...
muts <- muts[order(muts$position),]
muts$a_freq_old <- a_freq[match(muts$position, position)]
# I think what happened with the tree sequencing overlay:
# After recaptiation and adding neutral mutations, some neutral mutations
# overlapped with the QTL locations and "bumped up" the selected mutation from SliM
# to the next position 
# so QTLs with allele frequencies that don't match should be checked for this
# see issues at https://github.com/tskit-dev/pyslim/issues/25
# https://github.com/tskit-dev/msprime/issues/553 

getnonmatches_index <- which((muts$freq != muts$a_freq_old))
if(length(getnonmatches_index > 0)){
  print("There are some non-matching sites at:")
  getnonmatches_pos <- muts$position[ getnonmatches_index]
  print(getnonmatches_pos)
  print("These sites have been updated to:")
  muts2 <- muts
  for (i in 1:length(getnonmatches_pos)){
    muts_ind <- getnonmatches_index[i]
    af_ind <- which(position==muts$position[getnonmatches_index[i]])
    bb <- which(muts$freq[muts_ind]==a_freq[(af_ind-1):(af_ind+1)])
      # check if selected allele was moved one index up or down
    
    if (bb==1){ # was shifted one index earlier
      muts2$position[muts_ind] <- muts2$position[muts_ind] - 1
      muts2$a_freq_old[muts_ind] <- a_freq[af_ind - 1]
    }
    if (bb==3){ # was shifted one index later
      muts2$position[muts_ind] <- muts2$position[muts_ind] + 1
      muts2$a_freq_old[muts_ind] <- a_freq[af_ind + 1]
    }
...
rm(a_freq, position, chromosome)

### Unique identifer ####
muts$unique <- paste(muts$position, format(muts$a_freq_old,scientific = FALSE), sep="_")
# some mutations may be at the same site, so we use their position and frequency
# to determine a unique identifier for each one
...
if(!identical(muts$unique,
              as.character(final_df0$unique[which(final_df0$unique %in% muts$unique)]))){
  # exit there are more (or less) mutations in final_df0
  # then there is a problem
  print("Simulated mutations not represented in final df")
  break
}
...
### organize data for relatedness (annoying format) ####
genosub <- geno[keep2,]
genosub <- genosub
...
colnames(G_relate) <- rep(final_df0$position[keep2], each=2)
...
rm(i, abob, bbob, G_relate, G_relate2, geno, genosub, toclust, keep1, keep2,
   odd, poprelate2, rem_ind)
...
#plot(final_df0$a_freq_old, final_df0$a_freq_final, xlim=c(0.5,0.6))
...
final_df$vcf_ord_final <- 1:nrow(final_df)
head(final_df)

dim(G_sub2)
dim(vcf_filt)
muts <- muts[,-which(colnames(muts)=="freq")]

muts$a_freq_final <- final_df0$a_freq_final[match(muts$unique, final_df0$unique)]
# check that these are the same
if(!identical(muts$a_freq_final, final_df0$a_freq_final[match(muts$unique, final_df0$unique)])){
  print("Error: mutation table freqs not the same as final_df freqs")
  break;
}

rm(G, G_sub, final_df0)
...
muts$pa2[muts$type=="m2"] <- muts$selCoef[muts$type=="m2"]^2*
  (muts$a_freq_final[muts$type=="m2"])*
  (1-muts$a_freq_final[muts$type=="m2"])

muts$prop <- round(muts$pa2/sum(muts$pa2, na.rm=TRUE),5)
muts$He <- 2*muts$a_freq_final*(1-muts$a_freq_final)
#muts[order(muts$He),]
...
rm(which_pruned, G_coded, a_freq2)
#str(training)
...
dim(final_df_temp)
head(final_df_temp)
if(sum(final_df_temp$position!=sort(final_df_temp$position))>0){
  final_df_temp <- final_df_temp[order(final_df$position),]
  # just a check to make sure final_df is in correct order
}
...
head(final_df)
rm(final_df_temp, forfinal, FstDataFrame, P1, test)
...
spearmans <- apply(training$G, 1, FUN=function(x){
  a <- cor.test(x, ind$envi[which(ind$infinal)], method = "spearman", )
  return(a$estimate)
})
length(spearmans)
dim(final_df)
...
### Merge muts with final_df ####
final2 <- merge(final_df, muts, by=c("unique", "position","a_freq_old", "a_freq_final"), all.x=TRUE, all.y=TRUE)

if(sum(duplicated(final2$unique))>0){
  print("Error: duplicated unique locus names")
  break;
}

expected <- nrow(final_df) + length(which(!(muts$unique %in% final_df$unique)))
obs <- nrow(final2) 
if(expected != obs){
  print("Error: number of loci in merged final dataframe not right")
  break;
}  

if(nrow(final2[which(!(is.na(final2$type))),]) != nrow(muts)){
  print("Error in final dataframe, not all selected mutations represented")
  break
}
