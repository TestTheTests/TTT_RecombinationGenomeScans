...
### Convert VCF to 012 format ####
  ...
  final_df0 <- data.frame(pos=position, chrom=chromosome, 
                          a_freq_old = a_freq, 
  ...
  head(final_df0)
  final_df0[final_df0$pos > 135800 & final_df0$pos < 135805,]
  
  ### Mutation information ###
  muts$muttype[muts$type=='m2'] = "MT=2"
  muts$muttype[muts$type=='m4'] = "MT=4"
  muts$unique <- paste(muts$simID, muts$pos, muts$muttype, sep="_")
  
  final_df0$unique <- paste(seed, final_df0$pos, final_df0$muttype, sep="_")
  final_df0$unique[final_df0$muttype=="MT=1"] <- paste(final_df0$unique[final_df0$muttype=="MT=1"], 
                                                       final_df0$vcf_ord[final_df0$muttype=="MT=1"],
                                                       sep="_")
  
  dim(final_df0) # equal to dimention of VCF file
  final_df0$unique[final_df0$muttype=="MT=2"]
  muts$unique
  ...
  if (!(identical(final_df0$a_freq_old[final_df0$muttype=="MT=2"],
                  muts$freq[muts$type=="m2"]))){
    print("Error: allele frequencies not identical")
    break;
  }
  
  dim(final_df0[final_df0$muttype %in% c("MT=2", "MT=4"),])
  dim(muts)
  ...
  # indexes of highly related individuals
...
# First, remove the related individuals and re-calculate allele frequency
  ...
  plot(final_df0$a_freq_old, final_df0$a_freq_final)
...
# Next, decide which loci to filter based on the re-calculated allele frequency 
  ...
  identical(muts$freq_old, final_df0$a_freq_old[match(muts$unique, final_df0$unique)])
...
# Update mutation table to have updated allele frequencies after related individuals removed
  muts$pa2[muts$muttype=="MT=2"] <- muts$selCoef[muts$muttype=="MT=2"]^2*
    (muts$freq_final[muts$muttype=="MT=2"])*
    (1-muts$freq_final[muts$muttype=="MT=2"])
...
### Tables to write to file ####
    ...
    head(final_df)
    head(muts)
    muts <- muts[,-which(colnames(muts) %in% c("position", "type", "freq", "type2"))]

    print(c("dim final_df", dim(final_df)))
    print(c("dim muts", dim(muts)))
    
    
    (addmuts <- nrow(muts)-length(which(muts$unique %in% final_df$unique)))
    print(c("number of mutations added to final_df", addmuts))
    
    final_df$simID <- seed
    final_df_final <- merge(final_df, muts, all=TRUE)
    dim(final_df_final)
    
    final_df_final[final_df_final$muttype=="MT=2",]
    
    identical(final_df_final$a_freq_final[final_df_final$muttype=="MT=2"], 
              final_df_final$freq_final[final_df_final$muttype=="MT=2"])
    
    if(nrow(final_df_final)==(nrow(final_df)+addmuts)){
      print("final dataframe is correct size")
    }else{
      print("final data frame is not correct size")
      break;
    }

    head(final_df_final)
    if(
      (sum(final_df_final$muttype=="MT=2") != sum(muts$muttype=="MT=2")) |
      (sum(final_df_final$muttype=="MT=4") != sum(muts$muttype=="MT=4"))
    ){
      print("Error: wrong # of causal mutations in final df")
      break;
    }
    
    final_df_final$simID <- seed
    ...
    write.table(final_df_final, file = paste("../results_final/", seed, "_", type, "_ScanResults.txt", sep=""), row.names = FALSE)
