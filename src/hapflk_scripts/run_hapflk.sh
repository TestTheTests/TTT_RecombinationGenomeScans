...
do
	...
	# create pruned file
	${myDir}/pruneSNPs.pl -vcf $curVCF -scan $myDir/${i}_${simType}_"ScanResults.txt" -outfile $curPruned
	...
	numGroups=$(perl ${myDir}/vcf2plink.pl -vcf $curVCF -pop $curPop -outfile $allPlink 2>>logfiles/$i.log &)
	perl ${myDir}/vcf2plink.pl -vcf $curPruned -pop $curPop -outfile $prunedPlink 2>>logfiles/$i.log &
	 wait
	...
	perl ${myDir}/splitPedMapbyChrom.pl -infile $allPlink -outfile "${myDir}/plink_files/${i}/${i}" >>logfiles/$i.log 2>&1 &
	perl ${myDir}/splitPedMapbyChrom.pl -infile $prunedPlink -outfile "${myDir}/plink_files/${i}/${i}_PRUNED" >>logfiles/$i.log 2>&1 &
	wait
	
	################# Main hafplk analysis #############################
	### unpruned
	bash ${myDir}/runHapflkAllChroms.sh ${myDir}/plink_files/${i} ${myDir}/results/ALL/$i $i $kinshipAll $K >>logfiles/$i.log 2>&1
	# combine the results from the 10 chromosomes using an Rscript
	Rscript ${myDir}/concat_chr.R "${myDir}/results/ALL/$i/${i}_chr" "${myDir}/results/ALL/" $i
	### unpruned with pruned kinship
	bash ${myDir}/runHapflkAllChroms.sh ${myDir}/plink_files/${i} ${myDir}/results/ALL_PRUNED_KIN/$i ${i} $kinshipPruned $K >>logfiles/$i.log 2>&1
	Rscript ${myDir}/concat_chr.R "${myDir}/results/ALL_PRUNED_KIN/$i/${i}_chr" "${myDir}/results/ALL_PRUNED_KIN/" $i
	#### pruned with pruned kinship
	bash ${myDir}/runHapflkAllChroms.sh ${myDir}/plink_files/${i} ${myDir}/results/PRUNED/$i ${i}_PRUNED $kinshipPruned $K >>logfiles/$i.log 2>&1
	Rscript ${myDir}/concat_chr.R "${myDir}/results/PRUNED/$i/${i}_PRUNED_chr" "${myDir}/results/PRUNED/" $i
