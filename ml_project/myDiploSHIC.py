########################################################
#
# This is a slightly modified version of the diploSHIC
# code. I changed the names of the feature vectors and 
...
if argsDict['mode'] == 'train':
    ...
    bgs    = np.loadtxt(trainingDir+"Invers_BGS.fvec",skiprows=1)
    nDims  = int(bgs.shape[1] / numSubWins)
    bgs1   = np.reshape(bgs,(bgs.shape[0],nDims,numSubWins))
    invers = np.loadtxt(trainingDir+"Invers_Inversion.fvec",skiprows=1)
    inv1   = np.reshape(invers,(invers.shape[0],nDims,numSubWins))
    lowRC  = np.loadtxt(trainingDir+"Invers_lowRC.fvec",skiprows=1)
    lrc1   = np.reshape(lowRC,(lowRC.shape[0],nDims,numSubWins))
    neut   = np.loadtxt(trainingDir+"Invers_Neutral.fvec",skiprows=1)
    n1     = np.reshape(neut,(neut.shape[0],nDims,numSubWins))
    qtl    = np.loadtxt(trainingDir+"Invers_QTL.fvec",skiprows=1)
    q1     = np.reshape(qtl,(qtl.shape[0],nDims,numSubWins))
    ss     = np.loadtxt(trainingDir+"Invers_SS.fvec", skiprows=1)
    ss1    = np.reshape(ss, (ss.shape[0], nDims,numSubWins))
    varRC  = np.loadtxt(trainingDir+"Invers_varRC.fvec", skiprows=1)
    vrc1   = np.reshape(varRC, (ss.shape[0], nDims, numSubWins))

    both=np.concatenate((bgs1, inv1, lrc1, n1, q1, ss1, vrc1))
    y=np.concatenate((np.repeat(0,len(bgs1)),np.repeat(1,len(inv1)), np.repeat(2,len(lrc1)), np.repeat(3,len(n1)), 
                      np.repeat(4,len(q1)), np.repeat(5,len(ss1)), np.repeat(6,len(vrc1)) ))
    ...
    if (trainingDir==testingDir):
        ...
        y_train = y
        #testing data
        bgs    = np.loadtxt(testingDir+"Invers_BGS.fvec",skiprows=1)
        nDims  = int(bgs.shape[1] / numSubWins)
        bgs1   = np.reshape(bgs,(bgs.shape[0],nDims,numSubWins))
        invers = np.loadtxt(testingDir+"Invers_Inversion.fvec",skiprows=1)
        inv1   = np.reshape(invers,(invers.shape[0],nDims,numSubWins))
        lowRC  = np.loadtxt(testingDir+"Invers_lowRC.fvec",skiprows=1)
        lrc1   = np.reshape(lowRC,(lowRC.shape[0],nDims,numSubWins))
        neut   = np.loadtxt(testingDir+"Invers_Neutral.fvec",skiprows=1)
        n1     = np.reshape(neut,(neut.shape[0],nDims,numSubWins))
        qtl    = np.loadtxt(testingDir+"Invers_QTL.fvec",skiprows=1)
        q1     = np.reshape(qtl,(qtl.shape[0],nDims,numSubWins))
        ss     = np.loadtxt(testingDir+"Invers_SS.fvec", skiprows=1)
        ss1    = np.reshape(ss, (ss.shape[0], nDims,numSubWins))
        varRC  = np.loadtxt(testingDir+"Invers_varRC.fvec", skiprows=1)
        vrc1   = np.reshape(varRC, (ss.shape[0], nDims, numSubWins))

        both2=np.concatenate((bgs1, inv1, lrc1, n1, q1, ss1, vrc1))
        y_test=np.concatenate((np.repeat(0,len(bgs1)),np.repeat(1,len(inv1)), np.repeat(2,len(lrc1)), np.repeat(3,len(n1)), 
                      np.repeat(4,len(q1)), np.repeat(5,len(ss1)), np.repeat(6,len(vrc1)) ))
        X_test = both2.reshape(both2.shape[0],nDims,numSubWins,1)
      
    Y_train = np_utils.to_categorical(y_train, 7)    # changed 5 to 7 for my data
    ...
    output = Dense(7,name="out_dense",activation='softmax')(h) # changed orig value from 5 to 7
    ...
    classDict = {0:'bgSelection',1:'inversion',2:'lowRC',3:'neutral',4:'QTL', 5:'selectiveSweep', 6:'variableRC'}
    
    #output the predictions
    outputFile = open(argsDict['predictFileOutput'],'w')
    outputFile.write('chrom\tclassifiedWinStart\tclassifiedWinEnd\tbigWinRange\tpredClass\tprob(bgSelection)\tprob(inversion)' +
		     '\tprob(lowRC)\tprob(neutral)\tprob(QTL)\tprob(selectiveSweep)\tprob(variableRC)\n')
    for index, row in x_df.iterrows():
        outputFile.write('{}\t{}\t{}\t{}\t{}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\n'.format( row['chrom'],row['classifiedWinStart'],row['classifiedWinEnd'],row['bigWinRange'], \
        classDict[predictions[index]],preds[index][0],preds[index][1],preds[index][2],preds[index][3],preds[index][4],preds[index][5], preds[index][6]))
    outputFile.close
